# Final in required format - can modify no of rows and columns
"""
Snake RL in the requested format:

• Training: Pure NumPy grid environment + Q-learning (Q-table)
• Visualization: Pygame demo to replay the learned policy (no curses)
• AI framework: Start with Q-table; easy to upgrade to PyTorch DQN if the grid/state gets large

Run tips
--------
1) Train a Q-table policy:      python snake_qlearning_numpy_pygame.py --train
2) Visualize the learned policy: python snake_qlearning_numpy_pygame.py --play
...
DIRS = [(0, -1), (1, 0), (0, 1), (-1, 0)]  # Up, Right, Down, Left as (dx, dy)
...
class SnakeEnv:
    """Compact, tabular-friendly Snake environment.

    State encoding (11 bits -> 0..2047):
      0: danger_ahead
      1: danger_right
      2: danger_left
      3: moving_up
    ...
    def reset(self) -> int:
        ...
        self.hunger_limit = self.w * self.h * 2  # prevents endless loops
    ...
    def _is_collision(self, pos: Tuple[int, int]) -> bool:
        ...
        if x < 0 or x >= self.w or y < 0 or y >= self.h:
            return True
        if pos in self.snake:  # hitting own body
            return True
        return False
...
def parse_args():
    ...
    p.add_argument("--train", action="store_true", help="Train a Q-table and save to qtable.npy")
    p.add_argument("--play", action="store_true", help="Play using the saved Q-table with Pygame")
